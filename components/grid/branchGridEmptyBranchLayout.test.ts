...
describe('computeBranchGridLayout empty branch placeholders', () => {
  ...
  it('places stash nodes one row ahead and one lane below the anchor commit', () => {
    ...
    expect(stashNode!.column).toBe(anchorNode!.column + 1);
    const sameSlot = layout.renderNodes.filter(
  ...
  it('does not treat children of different parents as siblings at the same depth', () => {
    const rootSha = '1111111111111111111111111111111111111111';
    const parentASha = '2222222222222222222222222222222222222222';
    const parentBSha = '3333333333333333333333333333333333333333';
    const childASha = '4444444444444444444444444444444444444444';
    const siblingASha = '5555555555555555555555555555555555555555';
    const childBSha = '6666666666666666666666666666666666666666';
    const siblingBSha = '7777777777777777777777777777777777777777';
    const commits = [
      [rootSha, null, 'main'],
      [parentASha, rootSha, 'parent-a'],
      [parentBSha, rootSha, 'parent-b'],
      [childASha, parentASha, 'child-a'],
      [siblingASha, parentASha, 'sibling-a'],
      [childBSha, parentBSha, 'child-b'],
      [siblingBSha, parentBSha, 'sibling-b'],
    ] as const;
    const branches = commits.slice(1).map(([sha, , branch]) => makeBranch(branch, sha, 1, 'main'));
    branches.unshift(makeBranch('main', rootSha, 1));
    const layout = computeBranchGridLayout({
      branches,
      mergeNodes: [],
      directCommits: commits.map(([fullSha, parentSha, branch], index) => ({
        fullSha,
        sha: fullSha.slice(0, 7),
        branch,
        message: branch,
        author: 'test',
        date: `2026-05-25T15:${String(index).padStart(2, '0')}:00Z`,
        parentSha,
        parentShas: parentSha ? [parentSha] : [],
        childShas: commits.filter(([, candidateParentSha]) => candidateParentSha === fullSha).map(([sha]) => sha),
      })),
      unpushedDirectCommits: [],
      defaultBranch: 'main',
      branchCommitPreviews: Object.fromEntries(branches.map((branch) => [branch.name, []])),
      branchParentByName: Object.fromEntries(branches.map((branch) => [branch.name, branch.name === 'main' ? null : 'main'])),
      branchUniqueAheadCounts: Object.fromEntries(branches.map((branch) => [branch.name, 1])),
      manuallyOpenedClumps: new Set(),
      manuallyClosedClumps: new Set(),
      isDebugOpen: false,
      gridSearchQuery: '',
    ...
    const node = (sha: string) => layout.renderNodes.find((candidate) => candidate.commit.id === sha)!;
    expect(node(childASha).row).toBe(node(siblingASha).row);
    expect(node(childBSha).row).toBe(node(siblingBSha).row);
    expect(node(childASha).row).not.toBe(node(childBSha).row);
  });

  it('only attaches the first branch commit to the synthetic fork ancestor', () => {
    const rootSha = '1111111111111111111111111111111111111111';
    const firstSha = '2222222222222222222222222222222222222222';
    const middleSha = '3333333333333333333333333333333333333333';
    const tipSha = '4444444444444444444444444444444444444444';
    const branches = [
      makeBranch('main', rootSha, 1),
      makeBranch('feature', tipSha, 3, 'main'),
    ];
    const commits = [
      [rootSha, null, 'main'],
      [firstSha, rootSha, 'feature'],
      [middleSha, firstSha, 'feature'],
      [tipSha, middleSha, 'feature'],
    ] as const;
    const directCommits: DirectCommit[] = commits.map(([fullSha, parentSha, branch], index) => ({
      fullSha,
      sha: fullSha.slice(0, 7),
      branch,
      message: branch,
    ...
    const rootCluster = layout.clustersByBranch.get('main')!.find((cluster) =>
      cluster.commitIds.includes(`main:${rootSha}`),
    )!;
    expect(rootCluster.childShas).toEqual([firstSha]);
  });

  it('does not reserve a sibling descendant subtree between direct siblings', () => {
    const rootSha = '1111111111111111111111111111111111111111';
    const subtreeRootSha = '2222222222222222222222222222222222222222';
    const subtreeMiddleSha = '3333333333333333333333333333333333333333';
    const subtreeTipSha = '4444444444444444444444444444444444444444';
    const directSiblingSha = '5555555555555555555555555555555555555555';
    const commits = [
      [rootSha, null, 'main', 'author-1'],
      [subtreeRootSha, rootSha, 'subtree', 'author-2'],
      [subtreeMiddleSha, subtreeRootSha, 'subtree', 'author-3'],
      [subtreeTipSha, subtreeMiddleSha, 'subtree', 'author-4'],
      [directSiblingSha, rootSha, 'direct-sibling', 'author-5'],
    ] as const;
    const branches = [
      makeBranch('main', rootSha, 1),
      makeBranch('subtree', subtreeTipSha, 3, 'main'),
      makeBranch('direct-sibling', directSiblingSha, 1, 'main'),
    ];
    const layout = computeBranchGridLayout({
      branches,
      mergeNodes: [],
      directCommits: commits.map(([fullSha, parentSha, branch, author], index) => ({
        fullSha,
        sha: fullSha.slice(0, 7),
        branch,
        message: branch,
      ...
      unpushedDirectCommits: [],
      defaultBranch: 'main',
      branchCommitPreviews: { main: [], subtree: [], 'direct-sibling': [] },
      branchParentByName: { main: null, subtree: 'main', 'direct-sibling': 'main' },
      branchUniqueAheadCounts: { main: 1, subtree: 3, 'direct-sibling': 1 },
      manuallyOpenedClumps: new Set(),
      manuallyClosedClumps: new Set(),
      isDebugOpen: false,
      gridSearchQuery: '',
    ...
    const node = (sha: string) => layout.renderNodes.find((candidate) => candidate.commit.id === sha)!;
    expect(node(subtreeRootSha).row).toBe(node(directSiblingSha).row);
    expect(Math.abs(node(subtreeRootSha).column - node(directSiblingSha).column)).toBe(1);
  });
